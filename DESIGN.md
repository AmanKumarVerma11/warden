...
- **Guide (`site/guide/index.html`, Read mode).** A documentation surface in the same world, structured for comprehension: a sticky table of contents with scroll-spy, view-by-view and how-to rows on hairlines, note/tip callouts that carry a signal dot (never a colored border), and a native `<details>` FAQ accordion. Reading measure about 72ch, comfortable line-height, minimal motion.
